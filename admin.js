...
const compactToday = today.replaceAll("-", "").slice(2);
...
function orderFinancial(order) {
  ...
  const hasRecordedGrandTotal = order.grandTotal !== undefined && order.grandTotal !== null && order.grandTotal !== "";
...
function renderReports() {
  ...
  const settlements = Object.values(financials.reduce((result, row) => {
    const order = row.order;
    const key = order.introducer || "未指定介紹人";
    if (!result[key]) result[key] = { name: key, account: order.paymentAccount || {}, collected: 0, cost: 0, service: 0, source: 0, intro: 0 };
    result[key].collected += row.values.revenue;
    result[key].cost += row.values.cost;
    result[key].service += row.values.service;
    result[key].source += row.values.source;
    result[key].intro += row.values.intro;
    if (order.paymentAccount?.account) result[key].account = order.paymentAccount;
    return result;
  }, {}));
  qs("#introducer-settlements").innerHTML = settlements.map(row => `<tr><td><strong>${row.name}</strong><small>${row.account.bank || "銀行待設定"}・${row.account.account || "帳號待設定"}</small></td><td>${money(row.collected)}</td><td>${money(row.cost)}</td><td>${money(row.service)}</td><td>${money(row.source)}</td><td><strong>${money(row.intro)}</strong></td></tr>`).join("") || `<tr><td colspan="6"><strong>這個月份尚無已確認的代收款。</strong></td></tr>`;
  ...
  qs("#report-orders-table").innerHTML = monthOrders.map(order => {
    ...
    return `<tr data-order-id="${order.id}" class="${excluded ? "is-excluded" : ""}"><td><strong>${order.id}</strong><small>${order.lineDisplayName || order.customerName}・${excluded ? "未計入" : "已計入"}</small></td><td>${money(values.revenue)}<small>${order.couponCode ? `優惠碼 ${order.couponCode}・折抵 ${money(order.discountAmount)}` : "無優惠碼"}</small></td><td>${money(values.cost)}</td><td>${money(values.service)}</td><td>${money(values.addOnMargin)}</td><td>${money(values.intro)}</td><td>${money(values.source)}</td><td><strong>${money(values.unallocated)}</strong></td></tr>`;
...
function populateOrderForm(order) {
  ...
  const fields = ["status", "assignedTo", "paymentStatus", "docsStatus", "ticketStatus", "nextActionDate", "reservationExpiresAt", "purchased", "pending", "used", "refunded", "total", "grandTotal", "paymentAmount", "cost", "serviceFee", "introducer", "couponCode", "couponDescription", "discountAmount", "introducerProfit", "sourceProfit", "internalNotes"];
...
function updateProfitPreview() {
  ...
  const calculatedGrandTotal = numberValue(form.elements.total.value)
    ...
    + manualSale
    - numberValue(form.elements.discountAmount.value);
  form.elements.grandTotal.value = Math.max(0, calculatedGrandTotal);
...
function saveSelectedOrder(event) {
  ...
  order.grandTotal = Math.max(0, order.total + addOnSale(order) - numberValue(order.discountAmount));
...
function createOrder() {
  ...
  const order = {
    id: `PMR-${compactToday}-${next}`, customerId: "", lineUserId: "", lineDisplayName: "新客戶", customerName: "", phone: "", projectId: "待選專案", title: "尚未選擇專案", route: "", purchased: 1, pending: 0, used: 0, refunded: 0, paid: false, paymentStatus: "待匯款", paymentAmount: 0, total: 0, grandTotal: 0, cost: 0, serviceFee: 500, introducer: "", paymentAccount: { bank: "", owner: "", account: "" }, reservationExpiresAt: `${tomorrow}T23:59`, couponCode: "", couponDescription: "", discountAmount: 0, introducerProfit: 0, sourceProfit: 0, docsStatus: "尚未補件", ticketStatus: "尚未出票", assignedTo: "待分派", nextActionDate: tomorrow, expiry: "2027/12/31", status: "等待付款", statusType: "active", internalNotes: "", ticketSelections: [], addOns: [], createdAt: today.replaceAll("-", "/"), activity: [{ at: today.replaceAll("-", "/"), text: "客服手動建立訂單，專案保留 1 天" }]
  };
